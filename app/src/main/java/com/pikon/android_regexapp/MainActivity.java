...
//        ArrayAdapter<String> adapter = new ArrayAdapter<>( this, androidx.appcompat.R.layout.support_simple_spinner_dropdown_item, ValidationMode.getAllLabels() );
	...
	private boolean validate( ValidationMode mode, String input ) {
		...
		switch ( mode ) {
			...
			case PASSWORD:
				...
				break;
			case CENSORSHIP:
                result = validateCensorship( input, censor );
				break;
		}
		return result;
	}
	...
	private boolean validateCensorship( String input, ArrayList<String> forbidden ) {
		String regex = String.format( "\\b(?:%s|%s)\\b", String.join( "|", forbidden ), String.join( "|", forbidden.stream().map( s -> s.toUpperCase( Locale.ROOT ) ).toArray( String[]::new ) ));
